...
export function OnePitchCard({ adminEditor, img, order, initTitle, initDescription, newCard, id }: { adminEditor?: boolean, img: string, order: number, initTitle: string, initDescription: string, newCard?: true, id?: string }): JSX.Element {
    ...
    const removePitchCard = (id: string, imgStorageRef: string) => {
        return () => {
            ...
            // Delete the file
            deleteObject(pitchCardImgRefInStorage).then(() => {
        ...
        <div className="pitchcard-container" id={id}>
            <Box className="pitchcard-box">
                {adminEditor && id && !newCard ?
                    <Box sx={{ marginTop: "1rem", backgroundColor: "grey", textAlign: "left", width: "100%", display: "flex", alignItems: "center" }}>
                        <SvgIcon style={{ color: "red", cursor: "pointer" }} onClick={removePitchCard(id, img)} component={DeleteIcon} fontSize="large" />
                        <Box ><b>Remove entire card</b></Box>
                    </Box>
                    : null}

                {newCard ?
                    <Box minHeight="10rem" sx={{ opacity: "50%", textAlign: "center", width: "100%", backgroundColor: "grey" }}>
                ...
                {adminEditor ? <EditPitchardDescription onChange={handleDescriptionChange} initText={description} /> : <p>{description}</p>}
...
export function PitchCardsComponent({ pitchCardsDB, adminEdit }: { pitchCardsDB: DBWebsitePitchCardKey, adminEdit?: boolean }): JSX.Element {
    ...
    if (pitchCardsDB) {
        ...
        return (
            <>
                <Divider><h2>Edit pitchcards</h2></Divider>
        ...
        return (
            <Box className="wrapperOfImagesWithPitch">
                < OnePitchCard
                    ...
                    initDescription={"Rotary is a global network of 1.2 million neighbors, friends, leaders, and problem-solvers who see a world where people unite and take action to create lasting change – across the globe, in our communities, and in ourselves. Solving real problems takes real commitment and vision. For more than 110 years, Rotary's people of action have used their passion, energy, and intelligence to take action on sustainable projects. From literacy and peace to water and health, we are always working to better our world, and we stay committed to the end. Read more at <a href='https://www.rotary.org/en'>Rotary.org</a>."}
                />
                < OnePitchCard
                    adminEditor={adminEditor}
                    order={2}
    ...
    function handleChange(event: any) {
        ...
        uploadBytes(pitchCardRef, event.target.files[0]).then((snapshot) => {
            ...
            update(dbRef(db, `/websites/${WEBSITE_ID}/homepageContent/pitchCards/` + randomkey), {
                image: `${startURL}${storage.app.options.storageBucket}/o/${encodeURIComponent(snapshot.ref.fullPath)}?alt=media&token=${snapshot.metadata.downloadTokens}`, //pitchCardRef.fullPath, // url to storage
                //title: string;
                //description: string;
                id: randomkey,
                order: cardOrderNr
