import { Box, Button, Divider, FormControl, InputLabel, MenuItem, Select, TextField, ThemeProvider } from "@mui/material";
...
export function RegisterButtonComponent({ buttonContent }) {
    ...
    return (<Box textAlign="center" className="knapp-sektion">
            <div className="rotaryknapp">
                <ThemeProvider theme={eriksTheme}>
                    <Button color={buttonColor} href={formLink} variant="contained">{buttonText}</Button>
                </ThemeProvider>
            </div>
            <p>{buttonInfo}</p>
        </Box>);
}
async function saveButtonDataToDB(homepageButtonContent, websiteID) {
...
function handleButtonColorChange(event, websiteID) {
    update(ref(db, `websites/${websiteID}/homepageContent/button/`), { buttonColor: event.target.value });
}
export function EditRegisterButtonComponent({ buttonContent, websiteID }) {
    // Create state variables for each input field
    const [homepageButtonContent, setHomepageButtonContent] = useState(buttonContent);
 ...
 */
    ...
    useEffect(() => {
        ...
        onValue(readButtonContentFromDatabase, (snapshot) => {
            let buttonContentFromDB = snapshot.val() ? snapshot.val() : "";
            setHomepageButtonContent(buttonContentFromDB);
        });
    }, []);
    ...
    return (<>
            ...
            <Box sx={{ display: "inline-flex", minWidth: "10rem", flexDirection: "row", padding: "1rem" }} textAlign="center">
                <FormControl fullWidth>
                    ...
                    <Select labelId="demo-simple-select-label" variant="filled" name="buttonColor" id="demo-simple-select" value={homepageButtonContent && homepageButtonContent.buttonColor ? homepageButtonContent.buttonColor : "green"} label="Button Color" onChange={handleSelectChange}>
            ...
            <Box sx={{ display: "inline-flex", flexDirection: "row", padding: "1rem" }} component="form" textAlign='center' noValidate autoComplete="off" 
    ...
    onSubmit={() => saveButtonDataToDB(homepageButtonContent, websiteID)}>
                ...
                <TextField name="formLink" id="formLinkField" label="Link to form" sx={{ padding: "1rem" }} variant="filled" InputLabelProps={{ shrink: true }} value={homepageButtonContent.formLink} 
    //onChange={(e) => setFormLink(e.target.value)}
    onChange={handleChange}/>
                <TextField name="buttonText" id="buttonTextField" label="Button text" sx={{ padding: "1rem" }} variant="filled" InputLabelProps={{ shrink: true }} value={homepageButtonContent.buttonText} 
    //onChange={(e) => setButtonText(e.target.value)}
    onChange={handleChange}/>

                <TextField name="buttonInfo" id="buttonInfoField" sx={{ padding: "1rem" }} label="Button info" variant="filled" InputLabelProps={{ shrink: true }} value={homepageButtonContent.buttonInfo} 
