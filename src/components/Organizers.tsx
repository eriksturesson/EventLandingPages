import { DBWebsiteOrganizersKey, OrganizerObject } from "./utilsAndInterfaces/interfaces"

export function OneOrganizer({ organizer }: { organizer: OrganizerObject }): JSX.Element {
    return (
        <img className="organizer-logo" src={organizer?.logo} />
    )
}

export function OrganizersComponent({ organizers }: { organizers: DBWebsiteOrganizersKey }): JSX.Element {
    if (organizers && Object.keys(organizers).length > 0) {
        ...
        for (let organizer of Object.keys(organizers)) {
            if (organizer !== organizers.title) {
                ...
                arrayOfOrganizers.push(
                    <OneOrganizer organizer={organizerObject as OrganizerObject} />
                )
            }
        }
