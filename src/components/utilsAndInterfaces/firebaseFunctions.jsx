...
export async function readAndWriteToFirebase(args) {
    ...
    */
    try {
        if (args.method === 'update') {
            let myRef = ref(db, args.ref ? args.ref : '');
...
export async function firebaseDBPushAndReturnKey(args) {
    //firebase.database().ref().push().key; //Old v8
    if (args.ref) {
        return push(child(ref(db), args.ref)).key; //New v9
    }
    else {
